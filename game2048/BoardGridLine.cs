        ...
        protected override Geometry DefiningGeometry
        {
            get
            {
                ...
                using (StreamGeometryContext context = sg.Open())
                {
                    ...
                    {
                        context.BeginFigure(new Point(-2, VisiableHeight / board.RowCount * i), true, false);
                        context.LineTo(new Point(VisiableWidth + 2, VisiableHeight / board.RowCount * i), true, true);
                    }

                    for (int i = 0; i <= board.ColumnCount; i++)
                    {
                        context.BeginFigure(new Point(VisiableWidth / board.ColumnCount * i, -2), true, false);
                        context.LineTo(new Point(VisiableWidth / board.ColumnCount * i, VisiableHeight + 2), true, true);
                    }
                }
                sg.Freeze();
                gg.Children.Add(sg);
